---
name: episode-schema
description: The canonical data contract for grade-change episodes - file layout, column names, dtypes, metadata fields and label definitions. Use whenever writing, reading, generating, segmenting, labelling, or testing episode data, or when any module needs to exchange episode data with another.
...
`papermaking-process` skill — note that `stock_flow` is **m³/h**, not L/min.
...
dimension; `nominal_speed_m_min` is an operating parameter, not part of the embedding. Transitions are
represented as the vector `to_props - from_props`, so unseen grade pairs are retrievable
by proximity in this space.
...
- Any module that widens this schema must update this skill in the same commit.
