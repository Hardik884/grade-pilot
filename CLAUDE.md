...
2. **Every prediction and recommendation carries an evidence card.** No bare numbers
   reach the UI. See the `evidence-card` skill.
3. **All physical quantities carry explicit units.** See the `papermaking-process` skill
   for canonical units and valid ranges. Reject any value outside its physical range at
   the boundary of the module that produced it.
...
- Use plan mode for any change touching more than one module.
